...
Regular Expression, AkA REGEX are sequences for characters that are used to define a search pattern. These patterns have many applications commonly used in various string operations such as matching, replacing and searching for text within strings. we can use REGEX's to find text that conforms to specific patterns in order to effectively complete advanced text processing. Regular expressions are a powerful tool for processing and manipulation. REGEX's are commonly used for processing text files and can significantly cut down on the amount of code that is needed. REGEX is supported by all of the scripting languages. 
